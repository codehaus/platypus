...
public class MacroParser
{
    ...
    public int parse( char[] input, int parsePoint )
    {
        ...
        // verify first that it's really a macro (starts with: $ or [$ )
        if ( input[parsePoint] == '[' ) {
            parsePoint += 1;
        }
        ...
        //TODO: lookup for user macros when we implement those.

        // the +1 added below is for the end ] which is truncated from macro.
    ...
    public String  lookupSystemMacro( final String macro )
    {
        ...
        expandedMacro = gdd.getSysStrings().getString( macro );
        if ( expandedMacro == null ) {
            gdd.logWarning( gdd.getLit( "ERROR.MACRO_NOT_FOUND" ) +
                    ": " + macro );
        }
     ...
     */
    public String extractMacroName( final char[] input, int parsePoint  )
            throws IllegalArgumentException
    {
        ...
        if( input[parsePoint] == '$' || input[parsePoint] == '_' ) {
            ...
            gdd.logWarning( gdd.getLit( "INVALID_MACRO_NAME" ) + " " +
                    macro.toString() + gdd.getLit( "IGNORED" ));
